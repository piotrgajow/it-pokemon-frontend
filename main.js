...
function onQuizStarted() {
    updateScore(0);
    QUESTION_ELEMENT.classList.remove(CLASS_INVISIBLE);
}
...
function onNextQuestion(name) {
    ...
    QUESTION_ELEMENT.classList.remove(CLASS_CORRECT, CLASS_WRONG);
...
function onAnswerResult(result) {
    ...
    if (result.correct) {
        QUESTION_ELEMENT.classList.add(CLASS_CORRECT);
        RESULT_ELEMENT.innerText = 'You are correct!';
    } else {
        QUESTION_ELEMENT.classList.add(CLASS_WRONG);
...
function onQuizEnd() {
    state = STATE_NO_QUIZ;
    QUESTION_ELEMENT.classList.add(CLASS_INVISIBLE);
